...
@section('content')
    <form class="search-form" action="{{ route('admin.labTechs') }}" method="GET">
        <div class="form-row">
            <div class="col-md-9">
                <input type="text" class="form-control" name="search" placeholder="Search for a laboratory technician" />
            </div>
            <div class="col-sm-3">
    ...
    <a href="{{ route('admin.create.labTech') }}" class="btn btn-primary" style="margin-top:10px;">
        Add Laboratory Technician
    </a>

    <div class="table-responsive-md" style="margin-top: 30px">
        <table class="table">
            <caption>List of laboratory technicians</caption>
            <thead>
                <tr>
            ...
            <tbody>
                @foreach ($technicians as $technician)
                    <tr>
                        <th scope="row">{{ $technician->firstName . ' ' . $technician->lastName }}</th>
                        <td>{{ $technician->department->name }}</td>
                        <td>{{ $technician->phoneNumber }}</td>
                        <td>

                            @if ($technician->isContracted == 1)
                        ...
                        <td>
                            <a
                                href="{{ route('admin.edit-laboratoryTech', ['labTech_id' => Crypt::encrypt($technician->id)]) }}">
    ...
    @if ($tag === true)
        <div class="footer">
            <a href="{{ route('admin.labTechs') }}">
                <button class="btn btn-info">Get All Laboratory Technicians</button>
            </a>
        </div>
    @endif
    ...
    <script>
        document.getElementById('pagination').onchange = function() {
            window.location = "{!! $technicians->url(1) !!}&per_page=" + this.value;
        };
    </script>
@endsection
